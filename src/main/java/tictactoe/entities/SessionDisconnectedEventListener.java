...
public class SessionDisconnectedEventListener implements ApplicationListener<SessionDisconnectEvent> {
    ...
    public void onApplicationEvent(SessionDisconnectEvent sessionDisconnectEvent) {
        ...
        GameStateModel game = repository.findTopByPlayerOneIdOrPlayerTwoIdOrderByCreatedDesc(playerId, playerId);

        game.disconnectById(playerId);

        repository.save(game);
        template.convertAndSend("/tictactoe/gamestate/" + game.id, game);
    }
